...
import datn.web.dao.ManufacturerDao;
import datn.web.dao.ProductDao;
...
public class ProductServiceImp implements ProductService {
	...
	public Page<ShowProduct> getListProductByFilter(String nameSearch, String price, String manu, String sort,
		...
		if (status.equals("danh-sach")) {
			...
			preStatus = cb.equal(from.get("active"), 1);
		}
...
//		Predicate preActive = cb.equal(from.get("active"), 1);
		...
		if (check == 1) {
			cq.where(prePrice,  preDeleteDay, preStatus);
		} else if (check == 2) {
			cq.where(preManu, preDeleteDay, preStatus);
		} else if (check == 3) {
			cq.where(prePrice, preManu,  preDeleteDay, preStatus);
		} else {
			cq.where( preDeleteDay, preStatus);
		}
